...
settings$treebank <- "sv"
settings$ud <- sprintf("https://raw.githubusercontent.com/UniversalDependencies/%s/master", "UD_Swedish")
...
settings$wordvectors <- "https://s3-us-west-1.amazonaws.com/fasttext-vectors/wiki.sv.vec"
...
m <- udpipe_train(file = settings$modeloutput, 
                  ...
                  annotation_tokenizer = list(dimension = 24, epochs = 100, initialization_range = 0.1, 
                                              batch_size = 50, learning_rate = 0.01, dropout = 0.3, early_stopping = 1),
