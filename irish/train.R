...
settings$language <- "irish"
settings$treebank <- "ga"
settings$ud <- sprintf("https://raw.githubusercontent.com/UniversalDependencies/%s/master", "UD_Irish")
...
settings$wordvectors <- "https://s3-us-west-1.amazonaws.com/fasttext-vectors/wiki.ga.vec"
...
m <- udpipe_train(file = settings$modeloutput, 
                  ...
                  annotation_tokenizer = list(dimension = 64, epochs = 100, initialization_range = 0.1, 
                                              batch_size = 50, learning_rate = 0.005, dropout = 0.2, early_stopping = 1),
